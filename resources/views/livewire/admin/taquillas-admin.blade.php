<div>
    <x-layouts.menu-admin>
        <div class="bg-gray-100 max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8 mt-2">
            <span class="text-2xl font-semi-bold leading-normal">{{ __('Taquillas') }}</span>
            <div class="w-full flex mb-4 mt-2">
                <x-boton-primario wire:click="$set('open_crear', true)">
                    {{ __('Registrar Taquilla') }}
                </x-boton-primario>
            </div>
                ...
                <table class="w-full mt-4 table bg-white rounded-lg shadow mb-12" style="font-size: 0.8em">
                    <thead>
                        <tr>
                            ...
                            <th class="border-b-2 p-2 dark:border-dark-5 whitespace-nowrap font-normal text-gray-900 text-center" style="cursor:pointer" wire:click="order('moneda_id')">
                                {{ __('Moneda') }}
                                @if($sort == 'moneda_id')
                                    @if($direccion == 'asc')
                                        <i class="icofont icofont-caret-up float-right" style="font-size: 1.3em"></i>
                            ...
                            <th class="border-b-2 p-2 dark:border-dark-5 whitespace-nowrap font-normal text-gray-900 text-center" style="cursor:pointer" wire:click="order('cod_pais')">
                                {{ __('Código del Pais') }}
                                @if($sort == 'cod_pais')
                                    @if($direccion == 'asc')
                                        <i class="icofont icofont-caret-up float-right" style="font-size: 1.3em"></i>
                    ...
                    <tbody>
                        @foreach($taquillas as $datos)
                            <tr class="text-gray-700">
                                ...
                                <td class="border-b-2 p-2 dark:border-dark-5 text-center">
                                    {{ $datos->usuario_propietario->name }}
                                </td>
                                <td class="border-b-2 p-2 dark:border-dark-5 text-center">
            ...
            <x-slot name="content">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-2 my-2 p-2 rounded">
                    ...
                    <div class="mb-4">
                        ...
                        <select class="w-full border-gray-300 focus:border-indigo-300 focus:ring-indigo-300 rounded-md border border-primary pl-2 pr-2 py-2.5 focus:outline-none sm:text-sm" wire:model="moneda_id" disabled>
                    ...
                    <div class="mb-4">
                        <x-label value="{{ __('Código Pais (Ejemplo +58)') }}" />
                        <x-input wire:model.defer="cod_pais" type="text" class="w-full" />
                        <x-input-error for="cod_pais"/>
                    </div>
                    <div class="mb-4">
        ...
        <x-dialog-modal wire:model="open_crear">
            ...
            <x-slot name="content">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-2 my-2 p-2 rounded">
                    ...
                    <div class="mb-4">
                        <x-label value="{{ __('Moneda') }}" />
                        <select class="w-full border-gray-300 focus:border-indigo-300 focus:ring-indigo-500 rounded-md border border-primary pl-2 pr-2 py-2.5 focus:outline-none sm:text-sm" wire:model="moneda_id_crear" >
                            <option value="">Select...</option>
                            @foreach ($lista_monedas as $moneda)
                                <option value="{{ $moneda->id }}">{{ $moneda->moneda }}</option>
                            @endforeach
                        </select>
                        <x-input-error for="moneda_id_crear" />
                    </div>
                    <div class="mb-4">
                        <x-label value="{{ __('Caballos Mínimo') }}" />
                    ...
                    <div class="mb-4">
                        <x-label value="{{ __('Código Pais (Ejemplo +58)') }}" />
                        <x-input wire:model.defer="cod_pais_crear" type="text" class="w-full" />
                        <x-input-error for="cod_pais_crear"/>
                    </div>
                    <div class="mb-4">
        ...
        <x-dialog-modal wire:model="open_recarga">
            ...
            <x-slot name="content">
                ...
                <div class="mb-4">
                    ...
                    <select class="w-full border-gray-300 focus:border-indigo-300 focus:ring-indigo-300 rounded-md border border-primary pl-2 pr-2 py-2.5 focus:outline-none sm:text-sm" wire:model="operacion_recarga" >
