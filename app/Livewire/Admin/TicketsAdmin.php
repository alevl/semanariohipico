...
class TicketsAdmin extends Component
{
    ...
    public function render()
    {
        ...
        return view('livewire.admin.tickets-admin', compact('jugadas'));
    }
    ...
    public function anular($ticketId)
    {
        $buscar = CarrerasJugada::where('ticket', $ticketId)->first();
        $monto_anular = CarrerasJugada::where('ticket', $ticketId)->sum('monto_apuesta');

        $actualizar = CarrerasJugada::where('ticket', $ticketId)->update([
            'estatus_id' => 6,
        ]);
        
        if($buscar->origen_id == 2)
        {
            $actualizar = User::where('id', $buscar->taquilla_id)
            ->update([
                'monedero' => User::raw('monedero + '. $monto_anular),
            ]);

            date_default_timezone_set('America/Caracas');
            $fecha_anular = date('d')."/".date('m')."/".date('Y');
            $fecha_invertida_anular = date('Y').date('m').date('d');

            $crear = UsersMovimiento::create([
            'usuario_id'=>$buscar->taquilla_id,
            'fecha'=>$fecha_anular,
            'fecha_invertida'=>$fecha_invertida_anular,
            'operacion_id'=>2,
            'monto'=>$monto_anular,
            'descripcion'=>'Ticket anulado # '.$ticketId,
            ]);
        }
